...
class AddressModes(object):
    Illegal                   = ('Illegal', 1)                      # 44       .byte 0x44 ;illegal
    Inherent                  = ('Inherent', 1)                     # 00       nop
    ImmediateByte             = ('ImmediateByte', 2)                # 04 aa    mov a, #0xaa
    ImmediateWord             = ('ImmediateWord', 3)                # e4 aa bb movw a, #0xaabb
    Extended                  = ('Extended', 3)                     # 31 aa bb call 0xaabb
    Direct                    = ('Direct', 2)                       # 05 aa    mov a, 0xaa
    DirectWithImmediateByte   = ('DirectWithImmediateByte', 3)      # 85 aa bb mov 0xaa, #0xbb
    Register                  = ('Register', 1)                     # c8       inc r0
    RegisterWithImmediateByte = ('RegisterWithImmediateByte', 2)    # 88 aa    mov r0, #0xaa
    Pointer                   = ('Pointer', 1)                      # 07       mov a, @ep
    PointerWithImmediateByte  = ('PointerWithImmediateByte', 2)     # 87 aa    mov @ep, #0xaa
    Index                     = ('Index', 2)                        # d6 aa    movw @ix+0xaa, a
    IndexWithImmediateByte    = ('IndexWithImmediateByte', 3)       # 86 aa bb mov @ix+0xaa, #0xbb
    BitDirect                 = ('BitDirect', 2)                    # a0 aa    clrb 0xaa:0
    BitDirectWithRelative     = ('BitDirectWithRelative', 3)        # B0 FF 05    bbc 0xff:0, 0xe005
    Relative                  = ('Relative', 2)                     # fd 05    beq $e005
    Vector                    = ('Vector', 1)                       # e8       callv #0
...
def disassemble(rom, pc):
    opcode, disasm, mode = Opcodes[rom[pc]]
    instlen = mode[1]
    pc += 1
